...
$(document).ready(function(){
	...
	$('#frmSmeltReco').on('click', '#recovery_add_btn', function(){
		...
		rowCon += '</td>';
		rowCon += '<td>';
		rowCon += '<div><input type="text" name="con_rs_src_rowcc" id="con_rs_src_rowcc" class="form-control con_src_qty source-txtbox makeNil cvOn cvReq cvMaxLen" maxLength="150"></div><div class="err_cv"></div>';
		rowCon += '</td>';
		rowCon += '<td>';
            ...
            if(nilStatus == true){
                ...
                if(elArrRw == 3){
                    ...
                    if(metalEl == 'open'){
                        ...
                        $('#frmSmeltReco #con_rs_src_'+elRw).val('0');
...
function smeltRecoMainSymRw(){
	...
	smeltRecoMainRw('.con_src_qty','con_rs_src_','con_rs_src_',5);
